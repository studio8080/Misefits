...
- [ ] **A8 への広告掲載URLの追加提出**：`overseas-store.html`（オフィスコム）。A8 の「広告掲載URL管理」は別ウィンドウで開き、
      自動操作のタブでは「セッション切れ」になるので**本人が操作**する。9/25 は提出ページがタイムアウトして未完了。
...
- [ ] アクセシビリティの残り：`user-scalable=no` の解除検討（独自ピンチとの兼ね合い）、コントラスト不足、キーボードでの什器移動。
